...
class Drawer {
    ...
    constructor(ctx: CanvasRenderingContext2D, cellsSize: number) {
        this.ctx = ctx;
...
class Cell {
    ...
    constructor(color="#000000", isDead: boolean) {
        this.color = color;
        this.isDead = isDead;
    }
}
...
class CellMachine {
    ...
    start(interval_time, canvas, ctx, drawer: Drawer) {
     ...
     * @param {Drawer} drawer
     */
    bindStartStopStepControlsTo(el, canvas, drawer: Drawer) {
    ...
    bindSpawnAndDespawnControlsTo(canvas: HTMLCanvasElement, drawer: Drawer) {
...
function deepClone(cells2d): Cell[][] {
    const clone = []
